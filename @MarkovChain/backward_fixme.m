...
betaHat=zeros(nS,T);   
if ~fin
    %betaHat(:,T)=betaHat(:,1)./c(T); 
    betaHat(:,T)=betaHat(:,1).+1/c(T); 
    
else
    for i=1:nS
        betaHat(i,T)=A(i,nS+1)/(c(1)*c(T+1)); 
...
%for i=1:nS
    %for t=T-1:-1:1
 for t=T-1:-1:1           % swith t and i
     for i=1:nS
        summ=0;
        for j=i:nS
            %summ=summ+A(i,j)*pX(i,t+1)*betaHat(t+1,j); 
            summ=summ+A(i,j)*pX(j,t+1)*betaHat(t+1,j); 
            FinalProb=mean(FinalProb);
        end
        %betaHat(i,t)=(1/c(1))*summ; 
